...
class Server {
	...
	static void serve() {
		try {
			...
			Client.out.flush();
			CreateWindow.Draw(635, 660, "Server");
			if(DrawCanvas.d.wd[DrawCanvas.d.n[2]][0] == -1) {
				DrawCanvas.d.wd[DrawCanvas.d.n[2]][0] = DrawCanvas.d.wd[DrawCanvas.d.n[2]][1] = 0;
			}
			ArrayList<Integer> key = Enemy.brain(DrawCanvas.d);
			for(int i = 0 ; i < key.size() ; i ++) {
				DrawCanvas.d.key = key.get(i);
				Client.out.println(Deta.trans[DrawCanvas.d.key]);
				Client.out.flush();
			...
			while(true) {
				try {
					...
					if(Client.messe != null) {
						...
						if(DrawCanvas.d.np == 0) {
							key = Enemy.brain(DrawCanvas.d);
							for(int i = 0 ; i < key.size() ; i ++) {
								DrawCanvas.d.key = key.get(i);
								Client.out.println(Deta.trans[DrawCanvas.d.key]);
								Client.out.flush();
